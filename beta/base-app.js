(() => {
  ...
  function renderCatalog(){const query=($('#catalogSearch')?.value||'').trim();const all=window.NutritionHelper?.foods||[];const foods=query?(window.NutritionHelper?.search(query,80)||[]):all;$('#foodCatalog').innerHTML=foods.map(food=>{const sample=window.NutritionHelper.estimate(food,'');return `<article class="card food-card"><h3>${escapeHtml(food.name)}</h3><p>Base local • ${escapeHtml(food.standard)}</p><div class="food-row"><div class="food-kcal">${fmt(sample.calories)} <small>kcal</small></div><div class="food-macro"><span class="p">P: ${fmt(sample.protein,1)}g</span> · <span class="c">C: ${fmt(sample.carbs,1)}g</span> · <span class="f">G: ${fmt(sample.fat,1)}g</span></div></div><div class="food-actions"><button class="link-btn" data-catalog-add="${food.id}">${icon('plus')} Adicionar</button></div></article>`}).join('')||'<article class="card meal-empty">Nenhum alimento encontrado.</article>';$$('[data-catalog-add]').forEach(b=>b.onclick=()=>{switchView('diary');openNew('Outro',b.dataset.catalogAdd)})}
  ...
  function renderBackup(){const days=new Set(entries.map(e=>e.date)).size;$('#daysMonitored').textContent=`${days} ${days===1?'registro':'registros'}`;$('#snapshotCount').textContent=`${snapshots.length} ${snapshots.length===1?'snapshot disponível':'snapshots disponíveis'}`;$('#snapshotList').innerHTML=snapshots.length?snapshots.map(s=>`<div class="snapshot-item"><div><strong>${escapeHtml(s.label||'Ponto de restauração')}</strong><small>${new Date(s.timestamp).toLocaleString('pt-BR')}</small></div><button class="btn" data-restore-snapshot="${s.id}">Restaurar versão</button></div>`).join(''):'<div class="meal-empty">Nenhum ponto de restauração criado.</div>';$$('[data-restore-snapshot]').forEach(b=>b.onclick=()=>restoreSnapshot(b.dataset.restoreSnapshot))}
