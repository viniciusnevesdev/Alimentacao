(() => {
  ...
  function openCustomFood(prefill=''){
    ...
    $('#customFoodId').value='';
    $('#customFoodName').value=prefill||'';
    $('#customFoodRefQty').value=100;
    $('#customFoodUnit').value='g';
    $('#customFoodQuality').value=3;
  ...
  $('#food').addEventListener('input',()=>{selectedFood=null;estimateSource='';renderSuggestions($('#food').value);updatePreview()});$('#amount').addEventListener('input',()=>{updatePreview();if(selectedFood)applySuggestion(true)});$('#applySuggestion').onclick=()=>applySuggestion(false);['calories','protein','carbs','fat'].forEach(id=>$('#'+id).addEventListener('input',()=>{estimateSource=''}));$('#catalogSearch').addEventListener('input',renderCatalog);
  ...
  $('#scanPlaceholder').onclick=()=>{$('#catalogSearch').focus();$('#catalogSearch').scrollIntoView({behavior:'smooth',block:'center'})};$('#createFoodPlaceholder').onclick=()=>openCustomFood($('#catalogSearch').value.trim());
